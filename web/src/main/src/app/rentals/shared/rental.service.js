...
var RentalService = (function () {
  function RentalService(http) {
    this.http = http;
    this.rentalsUrl = 'http://localhost:8080/rentals';
  }
  ...
  RentalService.prototype.extractData = function (res) {
    var body = res.json();
    return body.rentals || {};
  };
  RentalService.prototype.handleError = function (error) {
    var errMsg;
  ...
  RentalService.prototype.getRental = function (id) {
    return this.getRentals()
      .map(function (rentals) { return rentals.find(function (rental) { return rental.id === id; }); });
  };
  RentalService.prototype.delete = function (rental) {
    debugger;
    var url = this.rentalsUrl/rental.id;
    console.log(url);
